...
const I18N = {
	...
	en: {
		...
		editor: {
			...
			pv_series: "PV series",
			pv_show_pv: "PV",
			pv_show_pool_load: "Pool load",
			pv_show_house_load: "House load",
			pv_show_surplus: "PV surplus pool",
			pv_show_bands: "PV bands (only without PV)",
			pv_show_pump_threshold: "Pump threshold",
			pv_show_aux_threshold: "Aux threshold",
			pv_legend: "PV legend",
			pv_legend_show_pv_group: "Legend item PV",
			pv_legend_show_pool_load: "Legend item Pool load",
...
class PoolControllerCard extends HTMLElement {
	setConfig(config) {
		...
		const next = {
			...
			pv_show_pool_load: config.pv_show_pool_load ?? DEFAULTS.pv_show_pool_load,
			...
			pv_legend_show_pool_load: config.pv_legend_show_pool_load ?? DEFAULTS.pv_legend_show_pool_load,
	...
	set hass(hass) {
		...
		if (content === "pv") {
			...
			const pvIds = this._getEntityIdsForContent("pv", effectiveConfig);
			if (!pvIds.length) {
				this._render();
				return;
			}
			const pvChanged = pvIds.some((entityId) => {
				const oldState = oldHass.states[entityId];
				const newState = hass.states[entityId];
				if (!oldState && !newState) return false;
				if (!oldState || !newState) return true;
	...
	async _render() {
		...
		switch (content) {
			...
			case "cost":
				...
				break;
			case "pv":
				blockHtml = this._renderPvBlock(data, effectiveConfig);
				break;
			case "controller":
			default:
	...
	_renderControllerScaffold(lang, c) {
		return `<div class="dial-container" data-role="pc-controller-root">
			<div class="dial" style="--accent:#8a3b32; --target-accent:rgba(138,59,50,0.3); --effective-accent:rgba(138,59,50,0.6)" data-dial data-role="pc-dial">
				<div class="ring">
	...
	_renderDialSvgMarkup(d) {
		...
		return `
			...
			${baseTargetAngle > d.dialAngle ? `<path class="ring-target" d="${this._arcPath(RING_CX, RING_CY, RING_R, RING_START_DEG + d.dialAngle, baseTargetAngle - d.dialAngle)}" />` : ""}
	...
	_prepareData(h, c, climate) {
		...
		const targetBaseEntityId = c.target_temp_base_entity || this._derivedEntities?.target_temp_base_entity || null;
		const targetEffectiveEntityId = c.target_temp_effective_entity || this._derivedEntities?.target_temp_effective_entity || null;
		const targetOffsetEntityId = c.target_temp_offset_entity || this._derivedEntities?.target_temp_offset_entity || null;
		const dynamicTargetProfileEntityId = c.dynamic_target_profile_entity || this._derivedEntities?.dynamic_target_profile_entity || null;
		const targetBase = targetBaseEntityId ? this._num(h.states[targetBaseEntityId]?.state) : target;
		const targetEffective = targetEffectiveEntityId ? this._num(h.states[targetEffectiveEntityId]?.state) : targetBase;
		const targetOffset = targetOffsetEntityId ? this._num(h.states[targetOffsetEntityId]?.state) : ((targetEffective != null && targetBase != null) ? (targetEffective - targetBase) : null);
		const dynamicTargetProfile = dynamicTargetProfileEntityId ? (h.states[dynamicTargetProfileEntityId]?.state || null) : null;
		...
		const effectiveTargetAngle = this._calcDial(targetEffective ?? targetBase ?? current ?? tc.min_temp, tc.min_temp, tc.max_temp);
		...
		return {
			...
			targetBaseEntityId,
			targetEffectiveEntityId,
			targetOffsetEntityId,
			dynamicTargetProfileEntityId,
			...
			hvac, hvacAction, climateOff, auxOn,
			...
			dialAngle, targetAngle, effectiveTargetAngle,
	...
	_getStyles() {
		return `<style>
			...
			* { box-sizing: border-box; }
			...
			.pv-legend-dot-pv-group { background:#2e7d32; }
			.pv-legend-dot-pool-load { background:#8a3b32; }
			...
			.pv-legend-dot-zone-off { background: rgba(244,67,54,0.85); }
			.pv-legend-dot-zone-stage1 { background: rgba(255,193,7,0.9); }
			.pv-legend-dot-zone-stage2 { background: rgba(76,175,80,0.9); }
		</style>`;
	}
	...
	_renderPvBlock(d, c) {
		...
		const auxThresholdEntity = c.power_saving_aux_threshold_entity || "";
		const bandLowEntity = c.pv_band_low_entity || "";
		...
		const showPoolLoad = c.pv_show_pool_load !== false;
		...
		const hasAny = !!(
			smoothedEntity || poolLoadEntity || houseLoadEntity || surplusEntity || pumpThresholdEntity || auxThresholdEntity
		);
		if (!hasAny) {
			return `<div class="info-badge">PV-/Stromsparen-Sensoren nicht gefunden</div>`;
		}
		...
		return `<div class="pv-block">
			<div class="cost-graph pv-graph"
				...
				data-show-pool-load="${showPoolLoad ? "1" : "0"}"
				...
				data-pool-load-entity="${poolLoadEntity}"
				data-pv-house-load-entity="${houseLoadEntity}"
				data-pv-surplus-entity="${surplusEntity}"
				data-pump-threshold-entity="${pumpThresholdEntity}"
				data-aux-threshold-entity="${auxThresholdEntity}"
				data-pv-band-low-entity="${bandLowEntity}"
	...
	_renderPvLegend(d, c) {
		...
		const num = (entityId) => {
			...
			return this._num(hass.states[entityId].state);
		};
		const fmtW = (value) => (value == null ? "n/v" : `${value.toLocaleString("de-DE", { maximumFractionDigits: 1 })} W`);
		...
		const showPumpThreshold = c.pv_show_pump_threshold !== false;
		const showAuxThreshold = c.pv_show_aux_threshold !== false;
		const showBandsEffective = showBands && !showPv;

		const legendPvGroup = c.pv_legend_show_pv_group !== false;
		const legendPoolLoad = c.pv_legend_show_pool_load !== false;
		...
		const pvNow = num(smoothedEntity);
		const poolLoadNow = num(poolLoadEntity);
		const houseNow = num(houseLoadEntity);
		const surplusNow = num(surplusEntity);
		const pumpThreshold = num(pumpThresholdEntity);
		const auxThreshold = num(auxThresholdEntity);
		...
		const labels = {
			de: {
				...
				pool_load: "Pool-Last",
				house_load: "Hauslast",
				surplus: "PV Überschuss Pool",
				off: "Aus",
				stage1: "Stufe 1 (Pumpe)",
				stage2: "Stufe 2 (Zusatz)",
				below: "unter",
				from: "ab",
			},
			en: {
		...
		if (view === "dynamic") {
			...
			if (pickerHost && pickerHost.getAttribute("data-rendered-key") !== key) {
				...
				try {
					...
					const refreshGraph = (ev) => {
						try {
							this._costLastPickerEvent = ev?.type || "picker-change";
							this._costLastPickerAt = new Date().toISOString();
	...
	async _attachPvGraph() {
		...
		const showPoolLoad = host.getAttribute("data-show-pool-load") !== "0";
		...
		const poolLoadEntity = host.getAttribute("data-pool-load-entity") || "";
		const houseLoadEntity = host.getAttribute("data-pv-house-load-entity") || "";
		const surplusEntity = host.getAttribute("data-pv-surplus-entity") || "";
		const pumpThresholdEntity = host.getAttribute("data-pump-threshold-entity") || "";
		const auxThresholdEntity = host.getAttribute("data-aux-threshold-entity") || "";
		const bandLowEntity = host.getAttribute("data-pv-band-low-entity") || "";
		const bandMidOffEntity = host.getAttribute("data-pv-band-mid-off-entity") || "";
		const bandMidOnEntity = host.getAttribute("data-pv-band-mid-on-entity") || "";
		const bandHighEntity = host.getAttribute("data-pv-band-high-entity") || "";
		const eff = this._withDerivedConfig(this._config || {});
		const powerSavingActiveEntityId = eff?.power_saving_active_entity || null;
		const powerSavingActive = powerSavingActiveEntityId ? this._isOn(this._hass?.states?.[powerSavingActiveEntityId]) : false;

		const key = [
			range,
			powerSavingActive ? "ps-on" : "ps-off",
			showPv ? "1" : "0",
			showPoolLoad ? "1" : "0",
			showHouseLoad ? "1" : "0",
			showSurplus ? "1" : "0",
			showBandsEffective ? "1" : "0",
		...
		try {
			...
			host.innerHTML = `<div class="info-badge">ApexCharts-Helfer konnten nicht geladen werden</div>`;
			return;
		}
		...
		if (showSurplus && surplusEntity) {
			series.push({ entity: surplusEntity, name: "PV Überschuss Pool", color: "#00897b", stroke_width: 2, opacity: 0.7 });
		}
		if (showBandsEffective && bandLowEntity) {
			series.push({ entity: bandLowEntity, name: "PV low", color: "#a5d6a7", stroke_width: 2, opacity: 0.55 });
		}
		if (showBandsEffective && bandMidOffEntity) {
			series.push({ entity: bandMidOffEntity, name: "PV mid (off)", color: "#81c784", stroke_width: 2, opacity: 0.6 });
		}
		if (showBandsEffective && bandMidOnEntity) {
			series.push({ entity: bandMidOnEntity, name: "PV mid (on)", color: "#66bb6a", stroke_width: 2, opacity: 0.65 });
		}
		if (showBandsEffective && bandHighEntity) {
			series.push({ entity: bandHighEntity, name: "PV high", color: "#43a047", stroke_width: 2, opacity: 0.75 });
		}
		...
		const stateNum = (entityId) => {
			if (!entityId || !this._hass?.states?.[entityId]) return null;
			return this._num(this._hass.states[entityId].state);
		};

		const pumpThreshold = stateNum(pumpThresholdEntity);
		const auxThreshold = stateNum(auxThresholdEntity);
		const valueCandidates = [
			stateNum(smoothedEntity),
			stateNum(poolLoadEntity),
			stateNum(houseLoadEntity),
			stateNum(surplusEntity),
			stateNum(bandLowEntity),
			stateNum(bandHighEntity),
			pumpThreshold,
			auxThreshold,
		].filter((v) => Number.isFinite(v) && v >= 0);
		const maxCandidate = valueCandidates.length ? Math.max(...valueCandidates) : null;
		const highestThreshold = [pumpThreshold, auxThreshold].filter((v) => Number.isFinite(v) && v > 0).reduce((acc, cur) => Math.max(acc, cur), 0);
		const bandMax = Number.isFinite(maxCandidate) ? Math.max(maxCandidate * 1.1, highestThreshold * 1.1) : null;
		const yaxisZones = [];
		if (Number.isFinite(bandMax) && bandMax > 0) {
			const pump = Number.isFinite(pumpThreshold) && pumpThreshold > 0 ? pumpThreshold : null;
			const aux = Number.isFinite(auxThreshold) && auxThreshold > 0 ? auxThreshold : null;
			if (pump != null) {
				yaxisZones.push({ y: 0, y2: Math.min(pump, bandMax), fillColor: "rgba(244,67,54,0.08)", opacity: 1 });
				if (aux != null && aux > pump) {
					yaxisZones.push({ y: pump, y2: Math.min(aux, bandMax), fillColor: "rgba(255,193,7,0.08)", opacity: 1 });
					yaxisZones.push({ y: aux, y2: bandMax, fillColor: "rgba(76,175,80,0.06)", opacity: 1 });
				} else {
					yaxisZones.push({ y: pump, y2: bandMax, fillColor: "rgba(255,193,7,0.08)", opacity: 1 });
				}
			} else if (aux != null) {
				yaxisZones.push({ y: 0, y2: Math.min(aux, bandMax), fillColor: "rgba(244,67,54,0.08)", opacity: 1 });
				yaxisZones.push({ y: aux, y2: bandMax, fillColor: "rgba(76,175,80,0.06)", opacity: 1 });
			}
		}
		...
		try {
			...
			host.innerHTML = `<div class="info-badge">ApexCharts-Karte nicht verfügbar (bitte custom:apexcharts-card installieren)</div>`;
		}
	}
		...
		switch (content) {
			case "pv":
				push(
					...
					c?.pv_surplus_for_pool_entity,
					c?.power_saving_pump_threshold_entity,
					c?.power_saving_aux_threshold_entity,
					c?.pv_band_low_entity,
					c?.pv_band_mid_off_entity,
					c?.pv_band_mid_on_entity,
					c?.pv_band_high_entity,
				);
				break;
			case "controller":
				push(
	...
	_hasRelevantChanges(oldHass, newHass, excludeEntityIds = null) {
		...
		const relevantEntities = [
			...
			this._config.pv_smoothed_entity,
			this._config.pv_house_load_entity,
			this._config.pv_surplus_for_pool_entity,
			this._config.power_saving_pump_threshold_entity,
			this._config.power_saving_aux_threshold_entity,
			this._config.pv_band_low_entity,
			this._config.pv_band_mid_off_entity,
			this._config.pv_band_mid_on_entity,
			this._config.pv_band_high_entity,
	...
	_withDerivedConfig(c) {
		...
		return {
			...
			pv_smoothed_entity: prefer('pv_smoothed_entity'),
			pv_house_load_entity: prefer('pv_house_load_entity'),
			pv_surplus_for_pool_entity: prefer('pv_surplus_for_pool_entity'),
			power_saving_pump_threshold_entity: prefer('power_saving_pump_threshold_entity'),
			power_saving_aux_threshold_entity: prefer('power_saving_aux_threshold_entity'),
			pv_band_low_entity: prefer('pv_band_low_entity'),
			pv_band_mid_off_entity: prefer('pv_band_mid_off_entity'),
			pv_band_mid_on_entity: prefer('pv_band_mid_on_entity'),
	...
	async _ensureDerivedEntities() {
		...
		this._derivedEntities = {
			...
			target_temp_base_entity: this._pickEntity(entries, "sensor", ["target_temp_base"]) || null,
			target_temp_effective_entity: this._pickEntity(entries, "sensor", ["target_temp_effective"]) || null,
			target_temp_offset_entity: this._pickEntity(entries, "sensor", ["target_temp_offset"]) || null,
			dynamic_target_profile_entity: this._pickEntity(entries, "sensor", ["dynamic_target_profile"]) || null,
...
class PoolControllerCardEditor extends HTMLElement {
		...
		<div class="wrapper">
			...
			<div class="row">
				...
				<select id="content-select" style="padding:8px; border:1px solid #d0d7de; border-radius:8px; background:#fff;">
					...
					<option value="pv">${_t(lang, "editor.content_options.pv")}</option>
				</select>
			</div>
			...
			<div class="row" id="pv-range-row" style="display:none;">
				...
				<select id="pv-range-select" style="padding:8px; border:1px solid #d0d7de; border-radius:8px; background:#fff;">
					...
					<option value="h4">${_t(lang, "editor.pv_timerange_options.h4")}</option>
			...
			<div class="row" id="pv-series-row" style="display:none;">
				...
				<div class="grid2">
					...
					<label style="display:flex;align-items:center;gap:8px;"><input id="pv-show-pool-load" type="checkbox" /><span>${_t(lang, "editor.pv_show_pool_load")}</span></label>
			...
			<div class="row" id="pv-legend-row" style="display:none;">
				...
				<div class="grid2">
					...
					<label style="display:flex;align-items:center;gap:8px;"><input id="pv-legend-show-pool-load" type="checkbox" /><span>${_t(lang, "editor.pv_legend_show_pool_load")}</span></label>
	...
	async _populateControllerSelect() {
		...
		try {
			const contentSelect = this.shadowRoot.querySelector('#content-select');
				...
				const pvShowPoolLoad = this.shadowRoot.querySelector('#pv-show-pool-load');
				...
				const pvLegendShowPoolLoad = this.shadowRoot.querySelector('#pv-legend-show-pool-load');
				...
				if (pvLegendShowPoolLoad) pvLegendShowPoolLoad.checked = this._config?.pv_legend_show_pool_load !== false;
				...
				if (pvShowPoolLoad) {
					pvShowPoolLoad.onchange = (e) => { this._updateConfig({ pv_show_pool_load: !!e.target.checked }); };
				}
				if (pvShowHouseLoad) {
				...
				if (pvLegendShowPoolLoad) {
					pvLegendShowPoolLoad.onchange = (e) => { this._updateConfig({ pv_legend_show_pool_load: !!e.target.checked }); };
				}
	...
	_buildPvApexConfigForClipboard() {
		...
		const showPoolLoad = merged.pv_show_pool_load !== false;
		...
		if (showBandsEffective && bandMidOffEntity) series.push({ entity: bandMidOffEntity, name: "PV mid (off)", color: "#81c784", stroke_width: 2, opacity: 0.6 });
		if (showBandsEffective && bandMidOnEntity) series.push({ entity: bandMidOnEntity, name: "PV mid (on)", color: "#66bb6a", stroke_width: 2, opacity: 0.65 });
		if (showBandsEffective && bandHighEntity) series.push({ entity: bandHighEntity, name: "PV high", color: "#43a047", stroke_width: 2, opacity: 0.75 });
	...
	_updateConfig(patch, renderOnly = false) {
		...
		const next = {
			...
			pv_show_pool_load: merged.pv_show_pool_load ?? DEFAULTS.pv_show_pool_load,
			...
			pv_legend_show_pool_load: merged.pv_legend_show_pool_load ?? DEFAULTS.pv_legend_show_pool_load,
